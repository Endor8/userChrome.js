    ...
    URILoadingHelper.openLinkIn = function(window, url, where, params) {
      if (where == "current") {
        ...
        if (params.targetBrowser) {
          w = params.targetBrowser.documentGlobal;
    ...
    init: function(){
      ...
      if (gBrowser && !/copytabLock/.test(func)) {
        func = func.replace(
          ...
          `if (aOtherTab.hasAttribute("tabLock")) {
              aOurTab.documentGlobal.gBrowser.lockTab(aOurTab, true);
            /*copytabLock*/
          }
          $&`
         );
      ...
      // CSS anwenden
        ...
        tab[tabLock] .tab-icon-lock{
          ...
          margin-left: 0px; /*Abstand links anpassen*/
...
*/
    ...
    tabContextMenu: function(){
      ...
      if (Services.appinfo.version.split(".")[0] >= 63)
        menuitem.setAttribute("label", "Tab(s) sperren");
      else
...
(function() {
  ...
  let frameScript = function() {
    addEventListener("click", onClick, true);  /*false Dadurch ist es nahezu identisch mit TabMixPlus, und JavaScript-Links werden von tabblock nicht beeinträchtigt. Wenn dies auf „true“ gesetzt ist, funktioniert event.preventDefault() von Erweiterungen möglicherweise nicht.*/
    ...
    let thresholdX = Services.prefs.getIntPref("ui.dragThresholdX", 4);
    let thresholdY = Services.prefs.getIntPref("ui.dragThresholdY", 4);
    let thresholdl = (thresholdX ** 2 + thresholdY ** 2) ** 0.5;
    function onClick(event) {
      if (event.button !== 0) return;
      if (event.altKey || event.ctrlKey || event.shiftKey) return;
      if (Math.abs(x - event.clientX) > thresholdX) return;
      let dl = ((x - event.clientX) ** 2 + (y - event.clientY) ** 2) ** 0.5;
      if (dl > thresholdl) return;

      let linkclick = sendSyncMessage("linkclick_isLockedTab", { })[0];
      if (!linkclick?.isLockedTab)
        return;
      /*Priorisiere Klickereignis-Listener von übergeordneten Elementen.*/
