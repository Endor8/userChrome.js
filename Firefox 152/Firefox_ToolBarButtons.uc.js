        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					if (event.button == 0) {
                    event.target.documentGlobal.Services.startup.quit( Ci.nsIAppStartup.eRestart | Ci.nsIAppStartup.eAttemptQuit );
                }
                if (event.button == 1) {
                    event.target.documentGlobal.Services.appinfo.invalidateCachesOnRestart();
                    event.target.documentGlobal.Services.startup.quit( Ci.nsIAppStartup.eRestart | Ci.nsIAppStartup.eAttemptQuit );
                }
				});
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                        event.target.documentGlobal.openTrustedLinkIn("about:config", "tab");
                }
				});
                return toolbaritem;
            }
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                        event.target.documentGlobal.openTrustedLinkIn("about:newtab", "tab");
                    }
				});
                return toolbaritem;
            }
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                        event.target.documentGlobal.SidebarController.toggle("viewBookmarksSidebar");
                    }				
					});
					return toolbaritem;
            }
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                        event.target.documentGlobal.SidebarController.toggle("viewHistorySidebar");
                    }
                });								 
                return toolbaritem;
            }
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                        event.target.documentGlobal.SidebarController.toggle("viewTabsSidebar");
                    }
				});								 
                return toolbaritem;
            }
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                        event.target.documentGlobal.DownloadsPanel.showDownloadsHistory();
                    }		
				});								 
                return toolbaritem;
            }
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                        event.target.documentGlobal.PlacesCommandHook.showPlacesOrganizer('AllBookmarks');
                                 }
				});			 
                return toolbaritem;
            }
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                        event.target.documentGlobal.BrowserCommands.reloadSkipCache();
                    }
				});		 
					return toolbaritem;
				}
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                    event.target.documentGlobal.openPreferences();
                }
			});			 
                return toolbaritem;
            }
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                        event.target.documentGlobal.openTrustedLinkIn("about:about", "tab");
                    }
					});								 
					return toolbaritem;
            }
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
					event.target.documentGlobal.BrowserCommands.pageInfo(null, 'mediaTab')
                    }
				});
                return toolbaritem;
            }
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                    event.target.documentGlobal.window.open( 'chrome://pippki/content/certManager.xhtml', 'mozilla:certmanager', "chrome,dialog,centerscreen,dependent,resizable=yes,width=700,min-height=760");
                    }
				});			 
                return toolbaritem;
            }
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
					event.target.documentGlobal.openTrustedLinkIn("about:logins", "tab")
                    }	
					});								 
                return toolbaritem;
				}
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                    event.target.documentGlobal.window.open('chrome://browser/content/sanitize.xhtml', 'Toolkit:SanitizeDialog', 'chrome,resizable=yes');
                                 }
				});	
                return toolbaritem;
            }
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                    event.target.documentGlobal.FullZoom.enlarge();
                    }				
					  });
                return toolbaritem;
            }
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
                    event.target.documentGlobal.FullZoom.reduce();
                    }				
					});
                return toolbaritem;
            }
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
					    ...
					    if (event.button == 0) { 
						event.target.documentGlobal.gBrowser.moveTabBackward(); 
					    }; 
					    if (event.button == 2) { 
						event.target.documentGlobal.gBrowser.moveTabForward(); 
					    }
					});
					...
					toolbaritem.addEventListener('wheel', event => {
						event.preventDefault();
						if (event.deltaY < 0) { 
                        event.target.documentGlobal.gBrowser.tabContainer.advanceSelectedTab(-1, true); 
                        } else { 
                        event.target.documentGlobal.gBrowser.tabContainer.advanceSelectedTab(1, true);
                    }
				});						 
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					if (event.button === 0) { 
						 ...
						 event.target.documentGlobal.FullZoom.reset(); 
        ...
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
				    ...
				    toolbaritem.addEventListener('click', event => {
					if (event.button == 0) { 
					event.target.documentGlobal.window.open("chrome://browser/content/preferences/dialogs/siteDataSettings.xhtml","cookie","chrome,dialog,centerscreen,dependent,resizable=yes,width=700,height=560");					
					}
				});										  
                return toolbaritem;
            }
        });
        // Firefox 'Über Firefox (A)'
        // Unicode-Konvertierung → Label: 'Über Firefox (A)', Tooltiptext: 'Über Firefox öffnen',
        CustomizableUI.createWidget({
            ...
            onBuild: function(aDocument) {
                ...
                toolbaritem.addEventListener("command", (event) => {
                    event.target.documentGlobal.openAboutDialog();
                });
                return toolbaritem;
            }
        });
            ...
            onBuild: function(aDocument) {
                ...
                for (let p in props)
					...
					toolbaritem.addEventListener('click', event => {
							event.preventDefault();
							if (event.button === 0) { 
								event.target.documentGlobal.openTrustedLinkIn("about:newtab", "tabshifted"); 
                            }; 
                            if (event.button == 1) {
                                event.target.documentGlobal.openTrustedLinkIn("about:config", "tab"); 
                            }; 
