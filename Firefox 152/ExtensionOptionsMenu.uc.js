...
// @include       main
// @charset       UTF-8
// @url			https://github.com/Endor8/userChrome.js/blob/master/Firefox%20127/ExtensionOptionsMenu.uc.js
// @version		  3.3.0 Fx152+ 
// @version		  3.2.0 Fx129+ Unterstützt neues Fenster und verschobene Fenster.
// @version		  3.1.9 Fx127 Das folgende Update behebt das Problem: 
// @Version             Der Befehl zum Öffnen der Optionsseite durch Rechtsklicken 
// @Version             auf ein Symbol zur Auswahl des Add-on-Managers und des 
// @Version             Add-ons und anschließendes Linksklicken wurde korrigiert 
// @Version             (neu geschrieben in BrowserAddonUI.openAddonsMgr).
// @version		  3.1.8 Übersprungen da es weitere Versionen mit demselben Namen gibt.
// @version       3.1.7 Anpassung an Firefox 127 von milupo
...
// @version       3.0.10 Es wurde ein vorhandenes Menü festgelegt, das nach FX52 angezeigt wird (aus dem 2Ch angemessenen Thread ausgeliehen).
// @version       3.0.9 Sie können nun Add-Ons festlegen, die Sie nicht anzeigen möchten (siehe Erklärung im Kommentarbereich), kopieren der Addon ID mit Alt + Linksklick. 
// @version       3.0.8 Fix für Neustart
// @version       3.0.7 Problem behoben, bei dem das Symbol nicht im Menü angezeigt wurde
// @version       3.0.6 Unter Berücksichtigung der Kompatibilität neu geschrieben. Schaltflächen werden standardmäßig in der Anpassungspalette platziert
// @version       3.0.5 CSS im Kommentarbereich wurde aktualisiert, um Bedienungsanweisungen im Tooltip anzuzeigen
// @version       3.0.4 Anzeigen der Add-on-Einstellungen funktionierte teilweise nicht, CSS wurde hinzugefügt, um zu verhindern, dass Drop-Marker im Menüfeld angezeigt werden.
// @Version       3.0.3 Schaltflächen kann nun über den Anpassen Dialog frei platziert werden. (unterstützt auch die Platzierung im Menüfeld Kommentar im *CSS Code hinzugefügt)
// ==/UserScript==
/*
...
var EOM = {
	...
	init: function() {
		...
		for (let i = 0, menu; menu = mMenus[i]; i++) {
			...
			if (!!menu.oncommand) {
        menuitem.addEventListener("command", (event) => {Cu.evalInSandbox(menu.oncommand, EOM.sb);});
      }
		};
